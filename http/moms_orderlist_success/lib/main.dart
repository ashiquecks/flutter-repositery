              ...
              return ListView.builder(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    return orderedProductView(
                      onpressed: () {},
                      context: context,
                      dueDate: data[index].dateOfExpair,
                      itemNo: data[index].productId,
                      price: data[index].price,
                      prdName: data[index].productName,
                      orderDate: data[index].dateOfExpair,
                      status: data[index].category,
                      
                      cusNam: data[index].productName,
                      ordAdd: data[index].productName,
                      cusPhon: data[index].productName,
                      ordTime: data[index].productName,
                      image: data[index].productImage,
                    );
                  });
            } else if (snapshot.hasError) {
              return Text("${snapshot.error}");
